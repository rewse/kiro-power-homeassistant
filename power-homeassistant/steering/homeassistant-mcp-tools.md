...
### Workflow 2: Create Automation

```
User: Create an automation that turns on the porch light at sunset
Agent: Uses ha_config_set_automation with proper YAML structure
       Creates automation with sun trigger and light action
...
Agent: Uses ha_get_automation_traces to check execution history
       Identifies trigger conditions, timing issues, or action failures
       Suggests fixes based on trace analysis
```

### Workflow 4: Dashboard Customization

```
User: Add a weather card to my dashboard
Agent: Uses ha_config_get_dashboard to read current config
       Adds weather card with proper YAML structure
